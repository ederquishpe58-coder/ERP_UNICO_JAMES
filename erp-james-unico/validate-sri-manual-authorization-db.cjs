...
const migrationPath = 'supabase/migrations/202609080005_sri_manual_authorization_recovery.sql';
const results = [];
...
async function main() {
  ...
  try {
    ...
    await test('Server cooldown rejects immediate repeated clicks without creating attempts', async () => {
      await login();
      await assert.rejects(() => claim(original750), error => error.code === '55P03'
        && error.message.includes('SRI_MANUAL_AUTHORIZATION_COOLDOWN') && JSON.parse(error.hint).retryAfterSeconds > 0);
    });
    ...
    for (const [name, options, patch, expected] of [
      ['Invoice 01 cannot enter manual 07 path', { type: '01' }, null, 'NOT_ELIGIBLE'],
      ['NC 04 cannot enter manual 07 path', { type: '04' }, null, 'NOT_ELIGIBLE'],
      ['Authorized 07 cannot requery', { status: 'AUTORIZADO' }, null, 'NOT_ELIGIBLE'],
      ['Definitive failure cannot enter uncertain recovery', {}, d => db.query("update sri_transmissions set error_class='SRI_AUTHORIZATION_IDENTITY_MISMATCH' where id=$1", [d.queryJob]), 'NOT_ELIGIBLE'],
      ['Unexhausted job retains existing retry contract', {}, d => db.query('update sri_transmissions set attempt_number=11 where id=$1', [d.queryJob]), 'NOT_ELIGIBLE'],
      ['No completion chronology fails closed', {}, d => db.query('update sri_transmissions set claimed_at=null,finished_at=null where id=$1', [d.queryJob]), 'NOT_ELIGIBLE']

...
async function saveUserAccess(client, actorUser, input, options = {}) {
  ...
  const companyAccess = resolvedCompanies.map((row, index) => {
    ...
    return {
      ...
      is_default: Boolean(row.isDefault) || index === 0
    };
  });
...
async function revokeUser(client, actorUser, input) {
  ...
  return { id: userId, revoked: true, canonicalAccess: data, operationId };
}

...
assert.doesNotMatch(updateAccessBody, /client\.auth\.admin\.(?:inviteUserByEmail|createUser|updateUserById|generateLink)/);
assert.match(updateAccessBody, /resolvedTarget\.created === true[\s\S]*client\.auth\.admin\.deleteUser\(target\.id\)/);
...
assert.match(apiSource, /async function createTargetUser[\s\S]*AUTH_EMAIL_OR_USERNAME_ALREADY_EXISTS[\s\S]*inviteUserByEmail/);
assert.doesNotMatch(
  apiSource.slice(apiSource.indexOf("async function createTargetUser"), apiSource.indexOf("async function saveUserAccess")),
  /updateUserById/
);
...
vm.runInNewContext(executableSource, {
  ...
  require(request) {
    ...
    if (request === "./sri/_lib/supabase-admin.cjs") {
      return { assertUserCapability() {}, getSupabaseAdmin() {}, getSupabaseUserContext() {}, bearerToken() {} };
    }

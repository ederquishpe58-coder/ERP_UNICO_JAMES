-- BLESS purchases only. Existing capability enforcement and internal transaction stay intact.
...
begin
 ...
 or v_config#>>'{purchaseAccounting,vatCreditAccountCode}' is distinct from '1.01.08' then raise exception 'PURCHASE_CANONICAL_MAPPING_REQUIRED'; end if;
 ...
 for v_line in select value from jsonb_array_elements(p_payload->'lines') loop
  ...
  if nullif(v_account,'') is null or v_account in ('5.3','5.5.04','2.1.01.01','1.1.04.01') then raise exception 'PURCHASE_LINE_ACCOUNT_REQUIRED: línea %',v_index; end if;
  begin perform public.erp_financial_v2_validate_account(p_company_id,v_account);
  exception when others then raise exception 'PURCHASE_LINE_ACCOUNT_REQUIRED: línea %, cuenta %',v_index,v_account; end;
  v_base:=(v_line->>'taxableBase')::numeric; v_rate:=(v_line->>'vatRate')::numeric; v_tax:=(v_line->>'vatValue')::numeric;
  if v_base is null or v_rate is null or v_tax is null or v_base<0 or v_rate<0 or v_tax<0 or abs(round(v_base*v_rate/100,2)-v_tax)>0.02 then raise exception 'PURCHASE_VAT_AMOUNT_MISMATCH: línea %',v_index; end if;
  if p_payload->>'source'='XML' then
   ...
   if v_line->>'vatRateAuthority' is distinct from 'EFFECTIVE_TABLE' or not exists (
    ...
    and (t.payload->>'rate')::numeric=v_rate and (t.payload->>'effectiveFrom')::date<=v_date
    and (nullif(t.payload->>'effectiveTo','') is null or (t.payload->>'effectiveTo')::date>=v_date)
   ) then raise exception 'PURCHASE_VAT_RATE_REQUIRED: parámetro efectivo por fecha'; end if;
  end if;
  if v_tax>0 and (v_credit is distinct from 'CREDIT' or v_support in ('02','04','07') or nullif(v_support,'') is null) then raise exception 'PURCHASE_VAT_WITHOUT_CREDIT_BLOCKED: tratamiento económico pendiente'; end if;
  v_journal:=p_payload#>'{journal,lines}'->(v_index-1);
  if v_journal->>'accountCode' is distinct from v_account or (v_journal->>'debit')::numeric is distinct from v_base or coalesce((v_journal->>'credit')::numeric,0)<>0 then raise exception 'PURCHASE_LINE_JOURNAL_MISMATCH'; end if;
 ...
 if (select coalesce(sum((line->>'debit')::numeric),0) from jsonb_array_elements(p_payload#>'{journal,lines}') with ordinality j(line,n) where n>v_index and n<jsonb_array_length(p_payload#>'{journal,lines}')) is distinct from v_total_tax then raise exception 'PURCHASE_VAT_JOURNAL_MISMATCH'; end if;
...
$function$;
revoke all on function public.erp_bless_purchase_validate_contract(uuid,jsonb) from public,anon,authenticated;

create or replace function public.erp_supplier_v2_post_purchase(p_operation_id uuid,p_company_id uuid,p_device_id text,p_payload jsonb,p_local_created_at timestamptz default now())
returns jsonb language plpgsql security definer set search_path=public,pg_temp as $function$
begin
 perform public.erp_u2c3_assert_mutation_capability('erp_supplier_v2_post_purchase',p_company_id,'{}'::jsonb);
 perform public.erp_bless_purchase_validate_contract(p_company_id,p_payload);

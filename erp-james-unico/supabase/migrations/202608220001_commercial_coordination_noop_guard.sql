...
-- PATCH de la RPC existente. Conserva su firma, locking, control de versión,
-- permisos e idempotencia, pero evita escribir/versionar cuando el cambio
-- solicitado ya coincide con el pedido canónico.
...
begin
  ...
  if p_patch ? 'daeNumber' then
    v_normalized_patch := jsonb_set(v_normalized_patch, '{daeNumber}', to_jsonb(btrim(coalesce(p_patch ->> 'daeNumber', ''))), true);
  end if;
  if p_patch ? 'hawb' then
    v_normalized_patch := jsonb_set(v_normalized_patch, '{hawb}', to_jsonb(btrim(coalesce(p_patch ->> 'hawb', ''))), true);
  end if;
  if p_patch ? 'airlineId' then
    v_normalized_patch := jsonb_set(v_normalized_patch, '{airlineId}', to_jsonb(btrim(coalesce(p_patch ->> 'airlineId', ''))), true);
  end if;
  if p_patch ? 'awb' then
    v_awb_digits := left(regexp_replace(coalesce(p_patch ->> 'awb', ''), '[^0-9]', '', 'g'), 11);
    v_normalized_patch := jsonb_set(
      v_normalized_patch,
      '{awb}',
      to_jsonb(case when length(v_awb_digits) > 3 then left(v_awb_digits, 3) || '-' || substr(v_awb_digits, 4) else v_awb_digits end),
      true
    );
  end if;
  ...
  for v_key, v_value in select key, value from jsonb_each_text(v_normalized_patch)
  loop
    if v_key = 'awb' then
      v_awb_digits := left(regexp_replace(coalesce(v_order.payload ->> 'awb', ''), '[^0-9]', '', 'g'), 11);
      v_current_value := case
        when length(v_awb_digits) > 3 then left(v_awb_digits, 3) || '-' || substr(v_awb_digits, 4)
        else v_awb_digits
      end;
    else
  ...
  insert into public.erp_operations_commands(
    ...
    v_normalized_patch, v_result, 'CONFIRMED', auth.uid(), p_device_id, coalesce(p_local_created_at, now())
  );
  return v_result;
end;
$$;

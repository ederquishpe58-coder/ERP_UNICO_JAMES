-- P0 phase 1 only: operation-context replay protection and four admin-read policies.
-- No business/config data, grants, profiles, sequence values or historical migrations change.
begin;
...
$function$
;
-- Own membership statuses are needed for login/company directory
ALTER POLICY memberships_select_self_or_admin ON public.user_company_memberships
  USING ((user_id = auth.uid()) OR public.erp_security_has_capability(company_id, 'admin.users.view'));

-- Own identity is needed for session bootstrap; other identity requires a shared authorized company
ALTER POLICY user_profiles_select_self_or_admin ON public.user_profiles
  USING ((user_id = auth.uid()) OR EXISTS (SELECT 1 FROM public.user_company_memberships target_membership WHERE target_membership.user_id = user_profiles.user_id AND public.erp_security_has_capability(target_membership.company_id, 'admin.users.view')));

-- Own route data remains readable for legacy compatibility; does not confer canonical capabilities
ALTER POLICY route_permissions_select_self_or_admin ON public.user_route_permissions
  USING ((user_id = auth.uid()) OR public.erp_security_has_capability(company_id, 'admin.users.view'));

-- Own access diagnostic records remain readable; others require company audit capability
ALTER POLICY access_audit_select_self_or_admin ON public.erp_access_audit_log
  USING ((user_id = auth.uid()) OR (company_id IS NOT NULL AND public.erp_security_has_capability(company_id, 'admin.audit.view')));

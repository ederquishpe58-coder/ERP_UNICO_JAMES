(function(){
  ...
  function permissions(){
    ...
    return {employeesManage:capabilities.has('payroll.employees.manage'),policiesManage:capabilities.has('payroll.performance_policies.manage'),
      ...
      approve:capabilities.has('payroll.roles.approve'),post:capabilities.has('payroll.roles.post'),print:capabilities.has('payroll.roles.print')};
  }
  ...
  function roleForPeriod(period,d=data()){const [year,month]=String(period||"").split("-").map(Number);const found=(d.periods||[]).find(p=>Number(p.year)===year&&Number(p.month)===month);return (d.roles||[]).find(r=>r.company_id===d.companyId&&String(value(r,"period_id","periodId"))===String(value(found,"period_id","periodId")))||null;}
  ...
  function renderApproved(route){
    ...
    return '<section class="payroll-page">'+header(route)+conceptsPanel(d)
      +(perms.accountingManage?'<form class="panel-card payroll-form-grid" data-payroll-v2-accounting><h3 class="payroll-form-span">Configuración contable de Nómina</h3>'
        ...
        +((d.conceptSettings||[]).some(c=>c.enabled&&['FOOD','ADVANCE','MANUAL_DISCOUNT','OTHER_DISCOUNTS'].includes(c.concept_code))?field('Cuenta de descuentos por pagar',accountSelect('deductionAccountCode',d.accountingSettings?.payroll_deduction_account_code||'','DEDUCTION'),'Se requiere cuando los descuentos aplicados utilicen este mapping.'):'')
      ...
      +(role?roleCard(role,perms):'<article class="panel-card">Sin rol para el período seleccionado.</article>')+'</section>';
  }
  ...
  async function runLocked(label,action){
    ...
    const selectedGeneration=ui.periodGeneration,periodAction=["Rol","Aprobación","Contabilización"].includes(label);

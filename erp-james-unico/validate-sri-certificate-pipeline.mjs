...
try{
 ...
 const select={value:'01:LOCAL',disabled:false},output={textContent:''},diagnostic={hidden:true},text={value:''};
 ...
 await zeroTest('UI dry-run available while SRI OFF, confirmed success only, file cleared',async()=>{assert.ok(erp.sriCertificatePrecheck.render().includes('Validar XML y firma'));erp.sriCertificatePrecheck.bind({querySelector:()=>panel});await dryClick();assert.match(output.textContent,/Prueba XML y firma válida/);assert.match(output.textContent,/Escrituras: 0/);assert.equal(input.value,'');assert.equal(dryButton.disabled,false);uiFail=true;await dryClick();assert.match(output.textContent,/no completada/);uiFail=false;});

(function(){
  ...
  function orderTotals(order) {
    ...
    const total = (order.lines || []).reduce((sum, line) => sum + number(line.bunches) * number(line.stemsPerBunch) * number(line.unitPrice), 0);
    return { boxes, bunches, stems, total };
  }
      ...
      <div class="commercial-v2-buyer-body">
        <section class="commercial-v2-buyer-group" aria-label="Información comercial del pedido">
          ...
          <div class="commercial-v2-buyer-grid commercial-v2-buyer-grid-client">
            ...
            <label><span class="commercial-v2-field-label">Marca / cliente final</span><select data-order-field="brandId" ${local ? "disabled" : ""}>${options(brands, order.brandId, row => row.finalClientName || row.name, local ? "No aplica en venta local" : "Seleccione")}</select></label>
  ...
  function boxBuilderPanel(appState) {
    ...
    const singleFields = mode !== "MIXTO_MANUAL" ? `<label>Variedad<select data-box-field="variety" data-excel-cell ${mode === "MIXTO_ABIERTO" ? "disabled" : ""}>${mode === "MIXTO_ABIERTO" ? '<option value="">Se completa en Cuarto Frío</option>' : catalogOptions(catalogs.varieties || [], draft.variety, "Seleccione variedad")}</select></label>` : "";
    return `<section class="panel-card commercial-v2-box-builder commercial-v2-box-builder-compact" data-box-builder ${flow.sessionFor(appState).boxBuilderOpen ? "" : "hidden"}>
      ...
      <div class="commercial-v2-builder-grid commercial-v2-box-builder-grid" data-excel-row>
        ...
        ${singleFields}
        ${mode !== "MIXTO_MANUAL" ? `<label>Medida<select data-box-field="length" data-excel-cell>${catalogOptions(catalogs.lengths || [], draft.length, "Seleccione medida")}</select></label><label>Ramos por caja<input type="number" value="${esc(draft.bunches)}" data-box-field="bunches" data-excel-cell></label><label>Tallos por ramo<input type="number" value="${esc(draft.stemsPerBunch)}" data-box-field="stemsPerBunch" data-excel-cell></label><label>Precio por tallo<input type="number" step="0.0001" value="${esc(draft.unitPrice)}" data-box-field="unitPrice" data-excel-cell></label>` : ""}
  ...
  function boxTable(order, catalogs) {
    ...
    return `<div class="table-wrap commercial-v2-lines"><table><thead><tr><th>Caja</th><th>Tipo</th><th>Variedad</th><th>Medida</th><th>Ramos</th><th>Tallos</th><th>Precio</th><th>PO</th><th>Acciones</th></tr></thead><tbody>
  ...
  function refreshOrderDependencies(container, appState) {
    ...
    if (brandField) {
      const brands = catalogs.brands.filter(row => !order.customerId || String(row.customerId) === String(order.customerId));
      brandField.innerHTML = options(brands, order.brandId, row => row.finalClientName || row.name, local ? "No aplica en venta local" : "Seleccione");
      brandField.disabled = local;
    }
    ["destinationCountry", "coldRoom", "daeNumber", "agencyId"].forEach(field => {
  ...
  function bindOrder(container, appState) {
    ...
    container.addEventListener("input", event => {
      ...
      if (orderField && !["customerId", "brandId", "saleType", "inventoryMode", "transportType", "daeNumber", "agencyId", "airlineId"].includes(orderField.dataset.orderField)) {
        ...
        if (orderField.dataset.orderField === "awb") {
          ...
          if (help) help.textContent = airline
            ...
            : (String(draft.awb || "").replace(/\D/g, "").length >= 3 ? "Prefijo no configurado en Líneas aéreas." : "Se reconoce con los tres primeros dígitos de la guía madre.");
        }
        return;
      }
    ...
    container.addEventListener("change", event => {
      ...
      if (["customerId", "brandId", "saleType", "inventoryMode", "transportType", "destinationCountry", "agencyId"].includes(field.dataset.orderField)) {
    ...
    if (view === "COORDINACION") {
      container.querySelectorAll('[data-coordination-field="awb"]').forEach(field => {
        ...
        if (upper(transport) === "AEREO") {
          ...
          field.setAttribute("aria-label", "MAWB: 3 dígitos, guion y 8 dígitos");
        } else {
  ...
  function renderAvailability(appState, options = {}) {
    ...
    return `<section class="page-header"><div><p class="section-kicker">DISPONIBILIDAD</p><h1>Inventario frente a pedidos guardados</h1><p>Guardar compromete la flor. Enviar a Cuarto Frío no descuenta una segunda vez.</p></div></section>
      ...
      <div class="table-wrap erp-table-scroll availability-v2-table-wrap"><table class="erp-data-table availability-v2-table"><thead><tr><th>Variedad</th><th>Medida</th><th>Calidad</th><th>Físico</th><th>Reservado</th><th>Empacado</th><th>Bloqueado</th><th>En pedido</th><th>Faltante</th><th>Disponible</th><th>Tallos disponibles</th></tr></thead><tbody>${rows.map(row => {
      ...
      }).join("") || `<tr><td colspan="11"><div class="empty-state compact">No existe inventario disponible para estos filtros.</div></td></tr>`}</tbody></table></div></section>`;
  }
  ...
  function bindPreorders(container, appState) {
    ...
    container.addEventListener("click", event => { const button = event.target.closest("button"); if (!button) return;
      if (button.hasAttribute("data-po-add-line")) { draft.lines.push(BlessERP.comercialData.createLine({ boxNumber: draft.lines.length + 1, unitPrice: 0 })); rerenderPage(); return; }
